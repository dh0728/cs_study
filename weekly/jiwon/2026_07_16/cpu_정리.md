...
#### 선점(Preemptive) vs 비선점(Nonpreemptive)이란?

한 프로세스가 CPU를 잡고 있을 때, **그 CPU를 누가 내려놓게 만드는가**에 대한 정책입니다.

| 구분 | 정의 |
| --- | --- |
| **Nonpreemptive (비선점)** | 프로세스가 CPU를 잡으면, **오직 자기 스스로 내놓을 때만** 반납. OS가 중간에 강제로 뺏을 수 없음 (I/O 요청, 종료 등 스스로 끝내는 경우에만 교체) |
| **Preemptive (선점)** | 프로세스가 아직 CPU를 쓰고 싶어도, **OS가 강제로 뺏어서** 다른 프로세스에게 줄 수 있음 (time slice 만료, 더 높은 우선순위 프로세스 등장 등) |

> 💡 비선점은 "내가 다 쓸 때까지 아무도 못 건드림", 선점은 "아직 안 끝났어도 순서가 되면 뺏길 수 있음"이라고 구분하면 쉽습니다. 뒤에 나오는 FCFS는 비선점, RR은 선점 알고리즘의 대표적인 예입니다.

이 구분이 실제로 적용되는 시점은 **프로세스가 상태를 바꿀 때**입니다 — 시험 단골!
...
| 프로세스 | 도착시간 | 실행시간 | 시작시간 | 대기시간 (시작−도착) |
| :---: | :---: | :---: | :---: | :---: |
| P1 | 0 | 5 | 0 | 0 |
| P2 | 1 | 3 | 5 | 4 |
| P3 | 2 | 8 | 8 | 6 |
...
- P1: 도착하자마자(0) 바로 실행
- P2: 1에 도착, P1이 끝나는 5까지 기다렸다가 실행 → 대기 4
- P3: 2에 도착, P2가 끝나는 8까지 기다렸다가 실행 → 대기 6
- P3가 16에 끝났는데 **P4는 아직 20에나 도착** → CPU가 **16~20구간 idle**
- P4: 도착하자마자(20) 바로 실행 → 대기 0

평균 대기 = (0+4+6+0)/4 = **2.5**

> 💡 도착 간격이 벌어지면(P3→P4 구간) ready queue가 텅 비면서 CPU가 노는 시간이 생깁니다. FCFS는 순서만 지킬 뿐, 다음 프로세스가 없으면 그냥 기다리는 것 말고는 방법이 없습니다.

> ⚠️ **핵심 문제 — Convoy Effect (호위 효과):** 긴 프로세스 뒤에 짧은 프로세스들이 줄줄이 갇혀서 기다리는 현상. 위 예시에서도 P3(burst 8)이 먼저 자리 잡고 있어서, 뒤에 짧은 프로세스가 도착했다면 P3가 끝날 때까지 무조건 기다려야 했을 것입니다. 도착 순서 하나(누가 먼저 줄을 서느냐)가 평균 대기시간을 크게 좌우합니다.
...
### 종합 예제 — 같은 데이터로 네 알고리즘 비교

같은 프로세스 집합을 FCFS, SJF(비선점), SRTF(선점형 SJF), RR(quantum=2)에 각각 넣어보면 알고리즘마다 결과가 어떻게 달라지는지 한눈에 비교할 수 있습니다.
...
> 💡 P1과 P2는 도착시간이 같으므로(0), 표에 적힌 순서(P1 → P2)대로 먼저 도착한 것으로 간주합니다.
...
0                   5    7      10   11
```
대기시간 = 시작시간 − 도착시간 → P1=0, P2=5, P3=6, P4=7
평균 대기 = (0+5+6+7)/4 = **4.5**

#### ② SJF (비선점)

시각 0엔 P1(5), P2(2) 중 P2가 더 짧아 P2 먼저. 시각 2엔 P1(5), P3(3) 중 P3 선택. 시각 5엔 P1(5), P4(1) 중 P4 선택.

```
| P2 | P3   | P4 | P1                |
0    2      5    6                   11
```
대기시간 → P1=6, P2=0, P3=1, P4=2
평균 대기 = (6+0+1+2)/4 = **2.25**

#### ③ SRTF (선점형 SJF)

시각 3에 P4(1)가 도착하는 순간, 실행 중이던 P3의 남은 시간(2)보다 P4가 더 짧아 **P3가 선점당함**.

```
| P2 | P3 | P4 | P3 | P1                |
0    2    3    4    6                   11
```
대기시간 → P1=6, P2=0, P3=2, P4=0
평균 대기 = (6+0+2+0)/4 = **2.0** ← 네 알고리즘 중 최소 (SJF는 optimal이므로 당연한 결과)

#### ④ RR (quantum = 2)

```
| P1 | P2 | P3 | P1 | P4 | P3 | P1 |
0    2    4    6    8    9    10   11
```
대기시간 → P1=6, P2=2, P3=6, P4=5
평균 대기 = (6+2+6+5)/4 = **4.75**

#### 비교 결과

| 알고리즘 | 평균 대기시간 | 평균 응답시간 |
| :--- | :---: | :---: |
| FCFS | 4.5 | 4.5 (비선점이라 대기시간과 동일) |
| SJF (비선점) | 2.25 | 2.25 (비선점이라 대기시간과 동일) |
| SRTF | **2.0** | 1.75 |
| RR (q=2) | 4.75 | 2.5 |

> ⚠️ 이 예제에서는 RR의 평균 대기시간이 FCFS보다도 나쁘게 나왔습니다. RR은 원래 대기시간을 줄이는 게 목표가 아니라 **응답시간을 고르게 낮추는 것**이 목표라서, 프로세스 수가 적고 burst가 짧은 이런 예제에서는 quantum마다 도는 오버헤드 성격의 대기가 오히려 두드러질 수 있습니다. 응답시간 열을 보면 RR이 FCFS(4.5)보다 훨씬 낫다는(2.5) 걸 확인할 수 있는데, 이게 RR이 실제로 잘하는 지표입니다.
